#ifndef HDXT_SERVER_H
#define HDXT_SERVER_H
#include <grpc++/grpc++.h>
...
namespace HDXT {
    class HDXTServiceImpl final : public RPC::Service {
    private:
        static rocksdb::DB *server_db1;
        static rocksdb::DB *server_db2;
	    static rocksdb::DB *server_db3;

    public:
        HDXTServiceImpl(const std::string db_path1, const std::string db_path2) {
            signal(SIGINT, abort);
	        ...
	        rocksdb::Status s3 = rocksdb::DB::Open(options, "/tmp/my.tmpsdb", &server_db3);
            if (!s3.ok()) {
        ...
        Status tupdate(ServerContext *context, ServerReader <UpdateRequestMessage1> *reader, ExecuteStatus *response) {
	        ...
	        std::string s1, s2, s3, s4, index, e;
            bool b;
             while (reader->Read(&request)) {
                index = request.index();
                s1 = request.ut();
        ...
        Status evict(ServerContext *context, ServerReader <EvictRequestMessage> *reader, ExecuteStatus *response) {
            ...
            while (reader->Read(&request)) {
                ...
                s2 = request.enc();
                if (index == "1"){
                    store(server_db3, s1, s2);
                } else {
                    rocksdb::Iterator *it = server_db2->NewIterator(rocksdb::ReadOptions());
                    rocksdb::Iterator *it2 = server_db3->NewIterator(rocksdb::ReadOptions());
                    std::string skey1, skey2;
                    std::string value1, value2, value;
                    //int t=0;
                    for (it->SeekToFirst(), it2->SeekToFirst(); it->Valid(), it2->Valid(); it->Next(), it2->Next()) {
                    //std::cout<<t<<std::endl;
                        ...
                        if (skey1!=skey2){
                            std::cout<<"error, error, error, does not match"<<std::endl;
                            break;
                        } else {
...
rocksdb::DB *HDXT::HDXTServiceImpl::server_db1;
rocksdb::DB *HDXT::HDXTServiceImpl::server_db2;
rocksdb::DB *HDXT::HDXTServiceImpl::server_db3;

void RunServer(std::string db_path1, std::string db_path2) {
    std::string server_address("0.0.0.0:50051");
    HDXT::HDXTServiceImpl service(db_path1, db_path2);
