...
namespace STORAGE {
   ...
   std::string Cutil::F(const std::string token, const byte *key) {
            ...
            if (token.size() <= 16){
                return std::string(cipher, 0, 16);
            } else {
                return Cutil::H1(cipher);
            }
    }
     ...
     static void AES_128_Key_Expansion(const unsigned char *userkey, void *key)
               ...
               int         e_len, int option)
        {
	        ...
	        *hashp = xor_block(*hashp, ta);
	        ...
	        //block ta = *ep;
            //block *ctp;
            //block *ctp = (block *)hash;
            int i = (e_len-1)/16; //i+1 block e_len = 17 i =1 e_len =16;i =0. j =0 e_len = 8 e_len = 35
            int j = e_len%16;   //remaining
        ...
        //std::string r = F2(s, key);
        std::string r = Cutil::F_aesni(key, s.c_str(), s.length(), 2);
        /*F_aesni(AES_KEY    *key,
               const void *e,
               int         e_len)*/
       // std::cout<<"size" << std::dec<<r.size()<<std::endl;
         //1 + r mod p-1
        ...
        //std::cout<<"p-1: "<<std::hex<<tmp<<std::endl;
        res = element + intr%tmp;
        //std::cout<<"res: "<<std::hex<<res<<std::endl;
        
        //flint::BigInt bir;
        //LibConversions::bytesToBigInt(ucr, 32, bir);
        /*flint::BigInt element;
        element.assign("1");
        //bir.assign
        //bir.add(flint::BigInt bigint = flint::BigInt(;);
        flint::BigInt tmp = p - flint::BigInt(1);
        std::cout<<"p-1: "<<tmp.toString()<<std::endl;
        flint::BigMod rmp(bir, p-flint::BigInt(1));
        res = element + rmp.getMantissa();
        std::cout<<"res: "<<res.toString()<<std::endl;*/
        /*if (res<element || res>tmp){
            std::cout<<"error"<<std::endl;
        }*/
        //std::string tmps = res.toHex();
        //std::cout<<tmps.size()<<std::endl;
    ...
    void  Cutil::Bytestoint(const std::string s, Integer& res){
        unsigned char *uc = (unsigned char *)s.c_str();
        std::stringstream bytesHexString;
        bytesHexString << std::hex << std::setfill('0');
        for(int i = 0; i < s.length(); i++) {
            bytesHexString << std::setw(2) << static_cast<int>(uc[i]);
        }
        std::string shex = bytesHexString.str()+"h";
        Integer r(shex.c_str());
        res = r;
    ...
    std::string Cutil::henc(const Integer& skey, const Integer& m, const Integer& n){
       ...
       // std::cout<<"size" << std::dec<<r.size()<<std::endl;
         //1 + r mod p-1
        /*unsigned char *ucskey = (unsigned char *)skey.c_str();
        std::stringstream bytesHexString;
        bytesHexString << std::hex << std::setfill('0');
        for(int i = 0; i < skey.length(); i++) {
            bytesHexString << std::setw(2) << static_cast<int>(ucskey[i]);
        }
        std::string skeyhex = bytesHexString.str()+"h";
        //std::cout<<hexstring<<std::endl;
        Integer int_skey(skeyhex.c_str());*/ 
